...
use crate::mtkey::{get_dispatch_keys, MTKey};
use common::cmdids::CmdIds;
use common::protos::{GetPlayerTokenRsp, PacketHead};
...
impl ClientServerPair {
    ...
    pub fn init_kcp(&mut self, conv: u32, token: u32) {
        let mut client = kcp::Kcp::new(conv, token, Source {});
        let mut server = kcp::Kcp::new(conv, token, Source {});

        client.set_nodelay(true, 10, 2, false);
        client.set_wndsize(256, 256);
        server.set_nodelay(true, 10, 2, false);
        server.set_wndsize(256, 256);
    ...
    fn decrypt_packet(&mut self, data: &mut Vec<u8>, is_client: bool) -> Option<Packet> {
        if let Some(session_key) = &self.session_key {
            ...
            if keys.contains_key(&first_bytes) {
                self.dispatch_key = Some(MTKey {
                    keybuf: keys[&first_bytes].to_vec(),
                });
    ...
    fn handle_parsed_packet(&mut self, p: &mut Packet) {
        ...
        let packethead = if p.header.len() > 0 {
            PacketHead::PacketHead::parse_from_bytes(&p.header).ok()
        } else {
            None
        };
        ...
        _ = match cmd {
            CmdIds::GetPlayerTokenRsp => {
                ...
                if let Some(v) = x {
                    ...
                    self.tokenrspserverseed = Some(u64::from_be_bytes([
