...
impl KeyBruteforce {
    fn guess(&self, test_buf: &[u8], ts: i64, server_seed: u64, depth: i32) -> Option<u64> {
        ...
        let key_suffix = [
            test_buf[test_buf.len() - 2] ^ 0x89,
            test_buf[test_buf.len() - 1] ^ 0xAB,
        ];
        ...
        for _ in 0..depth {
            ...
            let is_valid_suffix = key.keybuf[(test_buf.len() - 2) % key.keybuf.len()]
                == key_suffix[0]
                && key.keybuf[(test_buf.len() - 1) % key.keybuf.len()] == key_suffix[1];
            //
    ...
    pub fn bruteforce(&mut self, sent_time: u64, server_seed: u64, bytes: &[u8]) -> Option<u64> {
        ...
        for i in 0..3000 {
            ...
            if let Some(key) = self.guess(bytes, ts, server_seed, 10) {
    ...
    pub fn save(&self) {
        ...
        let n = if self.previous_seeds.len() > 1000 {
            self.previous_seeds.len() - 1000
        } else {
            0
        };
