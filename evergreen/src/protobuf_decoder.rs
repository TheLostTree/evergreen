...
impl DynProtoHandler {
    pub fn new() -> Self {
        DynProtoHandler {
            descriptors: None,
            descriptor_load: Some(std::thread::spawn(|| common::protos::load_dyn_protos())),
        }
    }

    pub fn decode(&mut self, cmdid: CmdIds, bytes: &[u8]) -> Option<Box<dyn protobuf::MessageDyn>> {
        let fdesc = self.get_descriptor(cmdid.clone())?;
        let msgdesc = fdesc.message_by_package_relative_name(&cmdid.to_string())?;
        let msg = msgdesc.parse_from_bytes(bytes).ok()?;
        Some(msg)
    }

    fn get_descriptor(&mut self, cmdid: CmdIds) -> Option<FileDescriptor> {
        ...
        if let Some(fdesc) = self.descriptors.as_ref().and_then(|x| x.get(&cmdid)) {
            ...
            // oh god....
            if let Some(descriptor_results) =
                self.descriptor_load.take().map(|f| JoinHandle::join(f))
            {
